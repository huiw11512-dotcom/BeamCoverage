...
The desktop 3D pages use an OpenGL display layer for smooth rotation and include real horizontal/vertical/height coordinate axes with tick labels and engineering units for S=S0 envelopes and normalized pattern views. Extreme physical aspect ratios are expanded only in the display layer, so narrow x/y ranges stay readable while axis tick labels remain true physical values.
...
Interactive desktop 3D pages use PyQtGraph OpenGL when available for smoother drag rotation, zoom, and pan. The calculation arrays are still generated by the existing Python core; OpenGL is only the display layer. The OpenGL view auto-centers the model, applies display-only aspect enhancement for extreme aperture/range ratios, balances x/y ranges that differ by large factors, and uses transparent low-value coloring for direction-pattern readability. Headless validation and machines without an OpenGL-capable context fall back to Matplotlib so smoke tests and report generation remain deterministic.
