...
Interactive desktop 3D pages use PyQtGraph OpenGL when available for smoother drag rotation, zoom, and pan. The calculation arrays are still generated by the existing Python core; OpenGL is only the display layer. Headless validation and machines without an OpenGL-capable context fall back to Matplotlib so smoke tests and report generation remain deterministic.
