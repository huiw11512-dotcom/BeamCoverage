...
def _default_release_readme() -> str:
    return (
        ...
        "- Interactive desktop 3D pages use PyQtGraph OpenGL when available for smoother drag rotation, zoom, and pan. The calculation arrays remain generated by the existing Python core; OpenGL is only the display layer, with auto-centering, display-only aspect enhancement for extreme aperture/range ratios, transparent low-value direction-pattern coloring, and a Matplotlib fallback for headless validation and report generation.\n\n"
